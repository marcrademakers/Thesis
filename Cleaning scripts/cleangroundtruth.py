...
def remove_unwanted_requirements(json_file, output_file, requirements_to_remove):
    """
    Removes JSON objects where:
      1. The 'requirement' field matches any entry in `requirements_to_remove`.
      2. The 'id' field is null (None in Python).

    Supports dictionaries where values are lists of objects.
    ...
    # Ensure JSON is a dictionary with lists inside
    if isinstance(data, dict):
        for key in data:
            if isinstance(data[key], list):  # Only process list values
                original_length = len(data[key])
                
                # Remove entries with matching requirements OR null IDs
                data[key] = [
                    item for item in data[key] 
                    if item.get("requirement") not in requirements_to_remove and item.get("id") is not None
                ]
        ...
        raise ValueError("JSON file structure is not supported. Expected a dictionary with lists.")
    ...
    print(f"🗑️ Removed {total_removed} requirement(s) (including null ID entries).")
